...
class WorkspaceController {
    ...
    static async getCurrentWorkspace(req, res) {
        try {
    ...
    static async inviteMember(req, res) {
        try {
            ...
            try {
                await PlanService_1.PlanService.enforceLimit(workspaceId, 'teamMembers');
            }
            catch (err) {
            ...
            await EmailService_1.EmailService.sendWorkspaceInvite(email, inviterMembership.workspace.name, inviterMembership.user.name || 'A colleague', invitation.id);
