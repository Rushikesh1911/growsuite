        ...
        try {
            ...
            if (task.assigneeId) {
                ...
                if (membership && membership.userId !== req.user.userId) {
                    ...
                    if (assigner) {
                        await EmailService_1.EmailService.sendTaskAssignmentAlert(membership.user.email, task.title, assigner.name || assigner.email, membership.workspace.name);
                    }
                }
            }
            socket_1.SocketService.emitToWorkspace(workspaceId, 'task_created', task);
            if (task.assignee && task.assignee.userId) {
                GoogleCalendarService_1.GoogleCalendarService.syncTaskToCalendar(task.assignee.userId, task);
            }
        ...
        try {
            ...
            if (assigneeId && assigneeId !== existingTask.assigneeId) {
                ...
                if (membership && membership.userId !== req.user.userId) {
                    ...
                    const assigner = await prisma.user.findUnique({ where: { id: req.user.userId } });
                    if (assigner) {
                        await EmailService_1.EmailService.sendTaskAssignmentAlert(membership.user.email, updatedTask.title, assigner.name || assigner.email, membership.workspace.name);
                    }
                }
            }
            socket_1.SocketService.emitToWorkspace(workspaceId, 'task_updated', updatedTask);
            if (updatedTask.assignee && updatedTask.assignee.userId) {
                GoogleCalendarService_1.GoogleCalendarService.syncTaskToCalendar(updatedTask.assignee.userId, updatedTask);
            }
    ...
    static async deleteTask(req, res) {
        try {
            ...
            if (existingTask.googleEventId && existingTask.assignee && existingTask.assignee.userId) {
                GoogleCalendarService_1.GoogleCalendarService.deleteTaskFromCalendar(existingTask.assignee.userId, existingTask.googleEventId);
            }
