...
class EmailService {
    ...
    static async sendWorkspaceInvite(toEmail, workspaceName, inviterName, invitationId) {
        if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
            console.log(`[EMAIL MOCK] 📧 Skipping actual email due to missing SMTP_USER/PASS`);
    ...
    static async sendNotificationEmail(toEmail, title, body) {
        if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
            console.log(`[EMAIL MOCK] 📧 Skipping actual email due to missing SMTP_USER/PASS`);
            return true;
        }
    ...
    static async sendTaskAssignmentAlert(toEmail, taskTitle, assignerName, workspaceName) {
        if (!process.env.SMTP_USER || !process.env.SMTP_PASS)
            return true;
        try {
            await transporter.sendMail({
    ...
    static async sendLeadOutreach(toEmail, subject, body, senderName, senderEmail) {
        if (!process.env.SMTP_USER || !process.env.SMTP_PASS)
            return true;
        try {
    ...
    static async sendInvoiceEmail(toEmail, invoiceNumber, amount, dueDate, clientName, workspaceName) {
        if (!process.env.SMTP_USER || !process.env.SMTP_PASS)
            return true;
        try {
            await transporter.sendMail({
