...
export class GoogleCalendarService {
  ...
  private static getOAuth2Client(user: any) {
  ...
  static async syncTaskToCalendar(userId: number, task: any) {
    try {
  ...
  static async deleteTaskFromCalendar(userId: number, googleEventId: string) {
    try {
      const user = await prisma.user.findUnique({ where: { id: userId } });
